...
class Graph {
    ...
    // run bfs from a vertex to check connectivity
    void bfs(vector<bool> &visited, set<pair<int, int> > &removed_edges) { // O((V+E)logV)
        ...
        while (!q.empty()) {
            ...
            for (int i = 0, siz = adj[u].size(); i < siz; i++) {
                int v = adj[u][i].first;
                if (removed_edges.find({u, v}) != removed_edges.end()) continue;
                if (removed_edges.find({v, u}) != removed_edges.end()) continue;

                // this edge has not been removed yet
                // can be considered
                if (!visited[v]) {
                    q.push(v);
                    visited[v] = 1;
...
public:
    ...
    ld find_MST_reverse_deletion(vector<pair<int, int> > &taken_edges) { // O(E^2logV)
        // sort the edges based on cost in non-increasing order of weights
        assert(costs.size() == m); // all edges have been added
        sort(costs.rbegin(), costs.rend()); // O(ElogE) = O(ElogV)

        // traverse through all the edges O(E)
        // maintain a set for the removed edges
        set<pair<int, int> > removed_edges;
        vector<bool> visited(n, 0); // O(V)

        for (int i = 0; i < m; i++) { // O(E^2 * logV)
            int idx = costs[i].second;
            int u = edges[idx].first;
            int v = edges[idx].second;

            visited.assign(n, 0); // O(V)
            // remove this edge
            removed_edges.insert({u, v}); // O(logE) = O(logV)
            // run bfs
            bfs(visited, removed_edges); // O((V+E)logV)
            // check_connectivity
            bool disconnected = 0;
            ...
            if (disconnected) {
                // (u, v) cannot be removed
                removed_edges.erase({u, v}); // O(logV)
            }
        }

        // all edges which could have been removed has been removed
        int siz = removed_edges.size();
        if (m - siz != n-1) {
            // no mst
            return -1.0;
        }
        ...
        for (int i = 0; i < m; i++) { // O(ElogV)
            ...
            if (removed_edges.find({u, v}) == removed_edges.end() && removed_edges.find({v, u}) == removed_edges.end()) {
                // this edge was taken in the mst
