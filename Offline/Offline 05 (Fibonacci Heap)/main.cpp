...
int main() {
    freopen("input1.txt", "r", stdin); // input from a given text file
    ...
    freopen("input2.txt", "r", stdin); // input from a given text file
    freopen("output.txt", "w", stdout); // output to a new text file
...
//        cout << "Time difference = " << chrono::duration_cast<chrono::nanoseconds> (end - begin).count() << "[ns]";
        ...
        if (cost == -1) {
            ...
            cout << path_len << ' ' << cost << ' ' <<
            chrono::duration_cast<chrono::nanoseconds> (bin_heap_end - bin_heap_begin).count() << ' ' <<
            chrono::duration_cast<chrono::nanoseconds> (fib_heap_end - fib_heap_begin).count() << "\n";
        }
    }
